...
import { getCharTableList, saveCharOrUpdate, enableCharRoles, disableCharRoles } from '@/api/system/userManage'
export default{
  data () {
    return {
      /* 字段设置必须值 */
  ...
  methods: {
    ...
    edit () { // 编辑
      ...
      if (sele.length >= 2) {
        ...
        this.editForm.code = sele[0].code
        this.editForm.name = sele[0].name
        this.editForm.status = status
        this.editForm.description = sele[0].description
      }
    },
    editSubmit () { // 确定编辑
