                            ...
                            <el-row :span="24">
                                ...
                                <el-col :span="6">
                                    <el-form-item label="用户昵称" label-width="30%">
                                        <el-input v-model="formData.name" placeholder="请输入登录手机号"></el-input>
                                    </el-form-item>
                                </el-col>
                                <el-col :span="6">
                ...
                <el-form :model="addForm" label-width="20%"  ref="addForm">
                    ...
                    <el-form-item label="分配角色" label-width="20%">
                        <el-select v-model="addForm.roleId" placeholder="请选择分配角色">
                            <el-option v-for="item in formSelect.roleId" :key="item.value" :label="item.label" :value="item.value"></el-option>
                        </el-select>
                    </el-form-item>
                </el-form>
  ...
  methods: {
    edit () {
      ...
      if (sele.length >= 2) {
        ...
        this.editForm.account = sele[0].account
        this.editForm.name = sele[0].name
        this.editForm.status = status
        this.editForm.password = sele[0].password
        for (let i in this.formSelect.roleId) {
          if (sele[0].roleId === this.formSelect.roleId[i].value) {
            this.editForm.roleId = sele[0].roleId
            break
          } else {
    ...
    editSubmit (editForm) {
      this.$refs[editForm].validate((valid) => {
        if (valid) {
          let status
          let _this = this
          if (typeof (this.editForm.status) === 'string') {
            if (this.editForm.status = '启用') {
              status = true
            } else {
              status = false
            }
      ...
      uSysRoleSelectList().then(res => {
        if (res.data.status) {
          for (let i in res.data.data) {
            let obj = {}
            obj.value = res.data.data[i].key
            obj.label = res.data.data[i].value
            sel[i] = obj
          }
          _this.formSelect.roleId = sel
        }
      })
